...
if isfield(opts, 'algorithmclass'), algorithmclass = opts.algorithmclass; end
...
for classf = classifierID
    ...
    if classf == 1
        ...
        for q= 1:numSplits
            ...
            if algorithmclass == 1
                Igain = evalDecision(Y, dec, u); % 计算信息增益
            elseif algorithmclass == 2
                Igain = gain_ratio(Y,dec,u); % 计算信息增益率
            else
                print('wrong opt.algorithmclass!');
            end
    ...
    elseif classf == 2
        ...
        for q= 1:numSplits
            ...
            r1= randi(D);
            r2= randi(D);
            w= randn(3, 1); % 返回一个3*1的矩阵，矩阵中的每个元素为标准正态分布中得到的随标量。
            
            dec = [X(:, [r1 r2]), ones(N, 1)]*w < 0; % 矩阵相乘以后，小于0的变为1，大于0 的变成0
            if algorithmclass == 1
                Igain = evalDecision(Y, dec, u); % 计算信息增益
            elseif algorithmclass == 2
                Igain = gain_ratio(Y,dec,u);
            else
                print('wrong opt.algorithmclass!');
            end
            if Igain>maxgain
    ...
    elseif classf==4
        ...
        for q= 1:numSplits
            ...
            if mod(q-1,5)==0
                x= X(randi(size(X, 1)), :);
                dsts= pdist2(X, x); % 计算两组观察值之间的成对距离。
                maxdsts= max(dsts);
                if length(dsts) == 1
                    mindsts = min(dsts);
                else
                    mindsts= min(dsts(find(dsts - min(dsts))));
                end
            end
...
function gain_ratio = gain_ratio(Y,dec,u) % 计算信息增益率
    Igain = evalDecision(Y,dec,u);
    YL= Y(dec);
    YR= Y(~dec);
    dv1 = length(YL)/length(Y);
    dv2 = length(YR)/length(Y);
    if dv1 == 0 || dv2 == 0
        iv = 1;
    else
        iv = -(dv1*log(dv1) + dv2 *log(dv2));
    end
    gain_ratio = Igain/iv;
end

function Igain= evalDecision(Y, dec, u) % 计算信息增益
...
function H= classEntropy(y, u) % 计算信息熵
